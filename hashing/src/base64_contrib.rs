...
pub fn bcrypt_hash64_decode(enc: &str, decbuf: &mut [u8]) -> Option<()> {
    let mut cbuild = 0u8;
    let mut cpos = 0;
    let mut dec_idx = 0;
    for b in enc.chars() {
...
pub fn decode_val(val: &str, len: usize) -> Option<u32> {
    let mut processed = 0;
    let mut s = 0u32;
    for b in val.chars() {
	let b = b as u32 - 0x20;
...
mod test {
    ...
    fn check_password_hash() {
        ...
        let mut maybe_data = vec![0; data.len()];
        bcrypt_hash64_decode(encoded, &mut maybe_data[..]);
        assert_eq!(crypt_hash64_encode(&data), encoded);
        assert_eq!(maybe_data[..], data);
    }
}
